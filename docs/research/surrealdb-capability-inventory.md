...
|---|---|
| `RrflowKvStore` opens an authenticated rrflowKV application format over the custom `rrd-lsm` database, persists WAL batches, validates read stamps, tracks an authenticated runtime log accumulator, and has crash/storage-full tests around WAL publication. C-01 through C-05 now establish the typed key, shared transaction, atomic semantic fan-out, direct reads, and one accepted pre-release execution shape. | This is a substantive durable substrate. C-06/C-07 still must finish selective page projection, physical-policy evidence, mapped-buffer lifetime, and final crash/maintenance qualification before the persistence objective can pass. |
...
| Many semantic values, catalogues, changes, index artifacts, and control records remain encoded payload bytes, but C-06 now persists them inside segment v5's ordered key/version spine and six Arrow-layout page buffers with safe mapped ownership, authenticated row-group filters, and explicit read/borrow/copy/allocation/filter evidence. | A columnar physical envelope and membership filter do not make JSON-like semantic values native graph/BM25/vector columns or prove DataFusion streaming. C-06 still owns the remaining physical-policy qualification; E/F own typed access paths, exact fallbacks, and stamped `RecordBatch` execution. |
