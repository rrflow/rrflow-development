...
| Many semantic values, catalogues, changes, index artifacts, and control records remain encoded payload bytes, but C-06's first slice now persists them inside segment v4's ordered key/version spine and six Arrow-layout page buffers with safe mapped ownership and explicit read/borrow/copy/allocation evidence. | A columnar physical envelope does not make JSON-like semantic values native graph/BM25/vector columns or prove DataFusion streaming. C-06 still owns selective provider projection and measured physical policy; E/F own typed access paths, exact fallbacks, and stamped `RecordBatch` execution. |
