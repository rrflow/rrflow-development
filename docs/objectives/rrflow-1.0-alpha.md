...
| OBJ-03 | Equivalent volatile and persistent semantics | Partial: rrflowMX and rrflowKV share one transaction port and C-03 semantic plan; rrflowKV recovers the exact multi-family write closure all-or-none; and C-04 proves equivalent authenticated, budgeted direct semantic-version reads, exact snapshots, and rrflowKV reopen/physical access without normal runtime-log reconstruction. Removal of pre-1.0 physical readers, the hybrid immutable format, final recovery/lifetime qualification, and the complete native graph/index/query corpus remain open. | The same transaction, snapshot, graph, index, and query corpus passes for rrflowMX and rrflowKV; rrflowKV additionally survives every recovery boundary. | C |
