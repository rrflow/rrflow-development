...
| POAM-008 | High | The low-level synchronous function/transaction-binding foundation is not the canonical engine-event, event-trigger, routine, host-event-adapter, or skill system. The unused `rrd-maintenance` crate additionally implements a fixed operation-specific state machine that directly owns a storage port, private scope/events, cursor-zero replay, JSON-wrapper records, hardcoded classes, and reduction policy with no focused tests. The package-command record remains a supporting policy at its pre-KB-05 flat path. | Automation can split into parallel authorities, bypass `RrdEngine`, and mistake fixed or untested machinery for generic restartable routines. | A-07, C-04, H-02, H-05, I-01 through I-07, J-01; KB-05 classification of `docs/package-workflows.md` | Direct naming/convergence plus restart, denial, idempotency, budget, activation, update, and uninstall conformance pass for generic packages; error resolution and context projection maintenance use the same I-03 routine authority; the standalone maintenance repository and direct-store path are absent; KB-05 moves the package-workflow record without duplicating its body. | Sequenced |
