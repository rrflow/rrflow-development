...
| C-06 | Hybrid immutable generation | `crates/persistence/rrd-lsm/tests/{segment,hybrid_segment,tiered_io,mvcc,compaction,snapshot_bundle}.rs`; `crates/persistence/rrd-store/tests/rrflow_kv_open.rs` | Partial: a sorted memtable flushes to one ordered key/version spine plus six typed Arrow-layout buffers; validated row/byte targets govern flush and compaction; fixed and generated mixed-family point/range/MVCC histories match independent exact state through reopen and protected compaction; malformed bytes fail closed; current bytes are frozen; prior formats fail; and mmap versus allocated ownership is measured. C-06g additionally proves a bounded selective projected stream over one owned pinned generation, exact multi-run/tombstone semantics, keys-only value-page omission, every request/operation/batch ceiling, cancellation/drop release, old-view survival through flush/compaction/GC, and distinct segment-open/startup-reconciliation/query evidence. Still required: C-06h adversarial/property/fuzz/fault and mixed-family qualification, then C-06i measured compression/value-placement/persisted-filter/cache choices. The DataFusion provider remains F-01/F-02 work. |
