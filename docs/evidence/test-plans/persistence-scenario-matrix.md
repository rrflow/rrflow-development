...
**Status:** active acceptance test plan; C-01 through C-06 are accepted, and C-07, E, and F remain open
...
| Accepted | The owning roadmap row cites retained output at an exact revision. C-01 through C-06 currently have this state. |
...
| CHAR-06 | `crates/persistence/rrd-store/tests/snapshot.rs::{retained_prefix_proofs_survive_later_commits_on_all_engines,rrflow_kv_snapshot_leases_pin_physical_manifests_until_release_or_expiry}` | Authenticated read stamps behave the same on rrflowMX/rrflowKV, and durable leases pin rrflowKV manifests. | C-04 removed normal full-log reconstruction and v5 pages now exist, but this test does not hold a borrowed Arrow page across compaction/reclamation; C-07 still owns that lifetime proof. |
...
| CHAR-14 | `crates/persistence/rrd-lsm/tests/write_diagnostics.rs`; `crates/persistence/rrd-store/tests/rrflow_kv_operator.rs::native_write_path_diagnostics_are_explicit_bounded_and_semantically_inert`; `crates/persistence/rrd-store/examples/engine_benchmark.rs` | Explicit bounded collection retains one accepted native batch record with separate transaction/encode/maintenance/WAL write-and-sync/memtable phases, adapter mutex waits, lower-bound byte growth, p99.9, and platform-labelled thread resource deltas; disabled writes retain no samples. | This diagnoses one write path and provides partial C-07/J-04 instrumentation only. It does not supply the real-process crash/ENOSPC/short-write matrix, pinned Arrow lifetime, concurrency/backpressure proof, fixed hardware, retained release provenance, or an end-to-end AI reasoning workload. |
...
C-01 through C-06 are `Accepted` by the owning roadmap. C-07 remains `Planned`;
later tests cannot substitute for its named observations.
...
| C-06 | Hybrid immutable generation | `crates/persistence/rrd-lsm/tests/{segment,hybrid_segment,projected_read_adversarial,page_cache,tiered_io,mvcc,compaction,snapshot_bundle}.rs`; `crates/persistence/rrd-lsm/examples/{rrflowkv_stress,rrflowkv_physical_policy}.rs`; `crates/persistence/rrd-lsm/fuzz`; `crates/persistence/rrd-store/tests/rrflow_kv_open.rs` | Accepted: a sorted memtable flushes to one ordered key/version spine plus six typed Arrow-layout buffers; validated row/byte targets govern flush and compaction; fixed and generated mixed-family point/range/MVCC histories match independent exact state through reopen and protected compaction; malformed bytes fail closed; current v6 bytes are frozen; versions 1 through 5 fail; and mmap versus allocated ownership is measured. C-06g proves a bounded selective projected stream over one owned pinned generation, exact multi-run/tombstone semantics, keys-only value-page omission, every request/operation/batch ceiling, cancellation/drop release, old-view survival through flush/compaction/GC, and distinct segment-open/startup-reconciliation/query evidence. C-06h adds a stable deterministic operation oracle, directed all-boundary fault/lifetime/resource programs, a configurable stress runner, and bounded AddressSanitizer-backed state-machine plus current segment-v6 fuzz targets. C-06i stores canonical ten-bit/seven-probe row-group filters and authenticates none/adaptive-LZ4 writer policy plus raw/LZ4 page codecs; corruption, invalid policy/length/envelope, and wrong decoded lengths fail. C-06j integrates selectable exact LRU and default scope-aware scan-resistant exact-byte caches. On clean revision `5b1c31d`, the same persisted eight-family corpus preserves manifest/semantic/projected-row identity and exact 1 MiB capacity while reducing post-scan hot-page loads from 48 to zero, with 18,200 same-scope suppressions and 48 cross-operation promotions. Value separation and semantic-family partitioning are explicitly rejected for this format. The DataFusion provider remains F-01/F-02 work; concurrency, recovery, maintenance, storage-full, and installed cross-platform lifetime remain C-07. |
| C-07 | Recovery and buffer lifetime | `crates/persistence/rrd-lsm/tests/mapped_page_lifetime.rs`; `crates/persistence/rrd-lsm/tests/write_diagnostics.rs`; existing failure/maintenance suites | WAL/manifest/compaction/reopen, ENOSPC, short write, checksum, torn pointer, and orphan cases lose no acknowledged batch; a pinned Arrow buffer remains valid after old-generation reclamation; maintenance/backpressure stays bounded. The write diagnostic now separates accepted-batch durability, maintenance, memory-pressure, lock, and scheduling evidence, but cannot close the remaining recovery/lifetime/concurrency matrix. |
