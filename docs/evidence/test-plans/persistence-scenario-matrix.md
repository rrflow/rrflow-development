...
| C-06 | Hybrid immutable generation | `crates/persistence/rrd-lsm/tests/{segment,hybrid_segment,projected_read_adversarial,tiered_io,mvcc,compaction,snapshot_bundle}.rs`; `crates/persistence/rrd-lsm/examples/{rrflowkv_stress,rrflowkv_physical_policy}.rs`; `crates/persistence/rrd-lsm/fuzz`; `crates/persistence/rrd-store/tests/rrflow_kv_open.rs` | Partial: a sorted memtable flushes to one ordered key/version spine plus six typed Arrow-layout buffers; validated row/byte targets govern flush and compaction; fixed and generated mixed-family point/range/MVCC histories match independent exact state through reopen and protected compaction; malformed bytes fail closed; current v6 bytes are frozen; versions 1 through 5 fail; and mmap versus allocated ownership is measured. C-06g proves a bounded selective projected stream over one owned pinned generation, exact multi-run/tombstone semantics, keys-only value-page omission, every request/operation/batch ceiling, cancellation/drop release, old-view survival through flush/compaction/GC, and distinct segment-open/startup-reconciliation/query evidence. C-06h adds a stable deterministic operation oracle, directed all-boundary fault/lifetime/resource programs, a configurable stress runner, and bounded AddressSanitizer-backed state-machine plus current segment-v6 fuzz targets with tracked seeds and ignored generated artifacts. C-06i stores canonical ten-bit/seven-probe row-group filters inside the authenticated index and now authenticates none/adaptive-LZ4 writer policy plus raw/LZ4 page codecs. Checksum-rewritten corruption, invalid policy/length/envelope inputs, and exact too-short/too-long decoded output fail; normal reopen reports filter and raw/compressed stored/logical bytes with zero semantic-page work; definite misses prune page loads; raw mmap pages borrow while compressed pages decode into bounded aligned owners; and none/adaptive semantic histories remain exact through reopen and protected compaction. Still required: value-placement, integrated mixed-workload interference, and cache-admission qualification. The DataFusion provider remains F-01/F-02 work. |
