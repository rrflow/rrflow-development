...
| rrflowKV immutable storage | One batch-v2, manifest-v2, and segment-v3 read path over LZ4-compressed row-record blocks with block indexes, compaction, and recovery; pre-1.0 batch/manifest/segment bytes fail unsupported before alternate decoding. C-05c proves one required physical dependency path, but the whole-executable audit still finds successful pre-release schema/vector representations above it. | C-05 must remove those remaining compiled alternate representations; only then may C-06 replace row blocks with the ordered key/version spine plus Arrow-compatible column pages. |
