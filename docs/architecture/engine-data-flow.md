...
| Transactions, identity, and audit | C-02 supplies one snapshot-isolation point/range/write transaction and one set of repositories for rrflowMX and rrflowKV. C-03a now prepares record/relation temporal state, both adjacency directions, generic projection work, runtime entry, outbox, audit, cursor, and outcome through one semantic plan. C-03b adds catalogue-digest/schema-bound scalar and unique entries plus BM25/vector source deltas to that plan; unique enforcement no longer replays records in rrflowQL. Function receipts and the complete effect-gap matrix remain absent. | C-03c/C-03d must join typed prepared function receipts and prove every WAL/publication/acknowledgement boundary; H-04/H-05 must prove the same authorization and audit contract across embedded and transport paths. |
