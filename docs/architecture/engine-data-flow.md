...
| Transactions, identity, and audit | C-02 supplies one snapshot-isolation point/range/write transaction and one set of claim, control, projection, runtime, and invocation repositories for rrflowMX and rrflowKV. Authenticated `ReadStamp`, `DataTransaction`, session/policy checks, commit receipts, and audit envelopes still have incomplete effect-level integration. | C-03 and H-04/H-05 must make the semantic record/graph/index/runtime/audit/outbox effects atomic and prove one authorization/audit contract across embedded and transport paths. |
