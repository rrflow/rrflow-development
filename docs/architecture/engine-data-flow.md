...
|---|---|---|
| rrflowKV writes and hot reads | Checksummed WAL frames, mutable MVCC version chains, snapshots, one consumed point/range/write transaction, authenticated block filters, a byte-bounded decoded-block cache, physical counters, and an AI-hotset benchmark. | Preserve the accepted C-02 transaction behavior while C-04/C-06/C-07 replace broad reads and the immutable format; F-05 decides the final cache from measurements. |
...
| Transactions, identity, and audit | C-02 supplies one snapshot-isolation point/range/write transaction and one set of repositories for rrflowMX and rrflowKV. C-03a prepares record/relation temporal state, both adjacency directions, generic projection work, runtime entry, outbox, audit, cursor, and outcome through one semantic plan. C-03b adds catalogue-digest/schema-bound scalar and unique entries plus BM25/vector source deltas. C-03c adds commit-bound governed-function receipts; the receipt, derived proposal, domain/index effects, semantic audit, outbox, cursor, and outcome use that same plan, and recovery consumes the prepared receipt without guest re-execution. | C-03d must prove every prepare/WAL/sync/visibility/acknowledgement and physical-size boundary. H-04/H-05 must complete same-stamp effect authorization plus identity/trace parity across embedded and transport paths. |
...
| Embedding and vectors | Deterministic local embedding, model/provenance binding, exact search, filtered planning, compact dense artifacts, HNSW, quantization, and accelerator differential checks exist. Canonical vector heads now retain their source commit coordinate and commit temporal versions plus collection/name/field pointer deltas atomically; no accelerator is built in that write path. | D-05/E-04/E-05/F-03 must consume those deltas through one collection catalogue, bind every derived artifact to its source cursor, and preserve exact fallback/reranking. |
| Graph, scalar, and BM25 | Current and temporal bidirectional adjacency keys now commit with relation changes. Schema-bound scalar/unique entries and exact BM25 old/new source fields commit with record changes and survive rrflowKV reopen. Current context and query execution still reconstruct broad snapshots; no native adjacency/scalar reader or incremental term dictionary/posting materializer is selected. | C-04/E-01 through E-03 must expose bounded stamped access, prove graph/scalar/BM25 results against exact oracles, and make stale/corrupt projections fall back or fail exactly as declared. |
