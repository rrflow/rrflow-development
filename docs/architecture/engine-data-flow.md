...
| rrflowKV immutable storage | One batch-v2, manifest-v2, and segment-v3 read path over LZ4-compressed row-record blocks with block indexes, compaction, and recovery; pre-1.0 batch/manifest/segment bytes fail unsupported before alternate decoding. | C-05 must finish whole-executable reader/opener/dependency closure; C-06 replaces row blocks with the ordered key/version spine plus Arrow-compatible column pages. |
