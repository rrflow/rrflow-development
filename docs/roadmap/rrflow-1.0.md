...
| [ ] | I-06 | Add previewable install/configure/retire/uninstall scaffolding for triggers, routines, host-event adapters, skills, and optional capability/activity bindings after their individual contracts pass. | `rrflow-cli`, adapters | Fresh/existing project tests show exact planned files/records/invocation closures, explicit consent, idempotent apply, retirement that blocks new work without deleting receipts, clean uninstall of only RRFlow-owned scaffolding, and no session-start loop. |
