...
| Estate desired/observed control and external-effect reconciliation | `rrd-estate/src/{lib,authority,reconcile,backup_job,backup_reconcile,recovery,local_process}.rs`; `rrd-store/src/control.rs`; `rrd-engine/src/engine/{estate,estate_control}.rs`; estate controller adapters and server estate-read handler | `rrd-estate/tests/{authority_catalogue,estate_authority,reconciler_recovery,backup_jobs,backup_reconciliation,recovery}.rs`; `rrd-store/tests/control_journal.rs`; `rrd-engine/tests/engine_authority.rs`; `rrd-server/tests/{local_estate_driver,http_process}.rs`; `rrflow-cli/tests/backup_controller.rs` | Preserve monotonic desired/observed state, fencing, prepared-before-effect, receipts, lost-ack convergence, activity, backup, and recovery invariants. Make `rrd-estate` pure; split the JSON aggregate into native typed records/relations/indexes; reconcile the duplicate operational-authority hierarchy; and execute every boundary through one installed, authenticated, stamped `RrdEngine` transaction. | A-07, C-01 through C-04, C-06, C-07, D-01, D-02, E-01, E-02, F-01, F-05, H-04, H-05, J-01 through J-05 |
...
|---:|---|---|
| 1 | `docs/instance-topology.md` | Freeze logical and physical instance topology after estate authority is clear. |
| 2 | `docs/local-process-driver-v1.md` | Reconcile the process adapter against the accepted security, estate, and topology boundaries. |
| 3 | `docs/rrd-deployment-modes-v1.md` | Author deployment profiles without creating alternate engines or persistence authorities. |
| 4 | `docs/rrd-cluster-m7.md` | Preserve useful distributed contracts, remove milestone authority, and expose unimplemented cluster behavior. |
| 5 | `docs/rrd-kubernetes-v1alpha1.md` | Keep only real Kubernetes API semantics and subordinate reconciliation to the engine contract. |
| 6 | `docs/rrd-security-bootstrap-v1.md` | Merge stable requirements into current security/install owners and remove the flat source; do not publish a task guide for unimplemented D-01 behavior. |
| 7 | `docs/clyffy-kernel-alpha.md` | Preserve only provider-neutral routing/orchestration requirements under RRFlow and remove the parallel-product framing. |
| 8 | `docs/package-workflows.md` | Reconcile package workflow semantics with the canonical event, trigger, routine, skill, and adapter boundaries. |
| 9 | `docs/rrd-functions-v1.md` | Preserve bounded function semantics while preventing a function runtime from becoming lifecycle authority. |
| 10 | `docs/rrd-rust-client-v1.md` | Establish the reference SDK behavior from the implemented Rust client and record open conformance gaps. |
| 11 | `docs/rrd-typescript-client-v1.md` | Reconcile the generated TypeScript projection against the shared contract and Rust reference behavior. |
| 12 | `docs/rrd-python-client-v1.md` | Reconcile the generated Python projection against the same contract and evidence. |
| 13 | `docs/rrd-go-client-v1.md` | Reconcile the generated Go projection without assigning Go orchestration authority. |
| 14 | `docs/rrd-java-client-v1.md` | Reconcile the generated Java projection against the shared SDK conformance boundary. |
| 15 | `docs/rrd-dotnet-client-v1.md` | Reconcile the generated .NET projection and close the SDK documentation set. |
| 16 | `docs/qdrant-capability-inventory.md` | Retain a source-pinned capability/reference inventory without importing Qdrant's product model. |
| 17 | `docs/surrealdb-capability-inventory.md` | Retain a source-pinned capability/reference inventory without importing SurrealDB's authority model. |
| 18 | `docs/rrflow-surrealdb-differential.md` | Preserve only reproducible claim-differential inputs and results after both source inventories are canonical. |
| 19 | `docs/anytype-ui-research.md` | Merge useful public-client/Connectome interaction requirements and remove UI product or lifecycle authority. |
| 20 | `docs/operations/ci.md` | Re-read the retained CI owner, verify its index and commands, and record the final KB-05 supporting-file disposition. |

After row 20, run the complete KB-05/A-06 acceptance corpus and change the
