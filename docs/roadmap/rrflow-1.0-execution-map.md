...
| Context projection maintenance draft | `rrd-maintenance/src/lib.rs` | no focused package test and no caller outside the package; pure validation behavior must receive new generic-routine acceptance tests before reuse | Preserve only digest-bound proposal, complete evidence-gate, review, generation, and rollback invariants. Replace the direct `StorageEngine`, private scope/event/repository, cursor-zero replay, JSON-wrapper records, fixed seven-stage lifecycle, eight hardcoded classes, and 50–75% policy with generic I-03 routine state and authorized `RrdEngine` operations; then remove the standalone crate without a wrapper. | A-07, C-04, H-02, H-05, I-03, J-01 |
...
| `rrd-maintenance::MaintenanceRepository` and its fixed maintenance state/event vocabulary | inventory reusable pure validation invariants, re-express context maintenance as one generic I-03 routine template through public `RrdEngine` operations, and remove the crate, direct store dependency, private scope, cursor-zero replay, JSON-wrapper records, hardcoded taxonomy, and reduction range with no alias or compatibility reader | A-07, C-04, H-02, H-05, I-03, J-01 |
