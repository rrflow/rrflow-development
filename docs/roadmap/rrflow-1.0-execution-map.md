...
| estate/cluster/operations | substantial local process, backup, recovery, cluster, Kubernetes, and operator-source foundations; `rrd-maintenance` is an unused standalone direct-store state-machine draft | retain the legitimate later-gate inventory; converge reusable maintenance validation into generic routines and remove its private repository/scope/events rather than accepting another semantic or storage authority |
...
|---:|---|---|
| 1 | `docs/estate-control-v1.md` | Separate desired/observed estate control from engine lifecycle and persistence authority. |
| 2 | `docs/instance-topology.md` | Freeze logical and physical instance topology after estate authority is clear. |
| 3 | `docs/local-process-driver-v1.md` | Reconcile the process adapter against the accepted security, estate, and topology boundaries. |
| 4 | `docs/rrd-deployment-modes-v1.md` | Author deployment profiles without creating alternate engines or persistence authorities. |
| 5 | `docs/rrd-cluster-m7.md` | Preserve useful distributed contracts, remove milestone authority, and expose unimplemented cluster behavior. |
| 6 | `docs/rrd-kubernetes-v1alpha1.md` | Keep only real Kubernetes API semantics and subordinate reconciliation to the engine contract. |
| 7 | `docs/rrd-security-bootstrap-v1.md` | Merge stable requirements into current security/install owners and remove the flat source; do not publish a task guide for unimplemented D-01 behavior. |
| 8 | `docs/clyffy-kernel-alpha.md` | Preserve only provider-neutral routing/orchestration requirements under RRFlow and remove the parallel-product framing. |
| 9 | `docs/package-workflows.md` | Reconcile package workflow semantics with the canonical event, trigger, routine, skill, and adapter boundaries. |
| 10 | `docs/rrd-functions-v1.md` | Preserve bounded function semantics while preventing a function runtime from becoming lifecycle authority. |
| 11 | `docs/rrd-rust-client-v1.md` | Establish the reference SDK behavior from the implemented Rust client and record open conformance gaps. |
| 12 | `docs/rrd-typescript-client-v1.md` | Reconcile the generated TypeScript projection against the shared contract and Rust reference behavior. |
| 13 | `docs/rrd-python-client-v1.md` | Reconcile the generated Python projection against the same contract and evidence. |
| 14 | `docs/rrd-go-client-v1.md` | Reconcile the generated Go projection without assigning Go orchestration authority. |
| 15 | `docs/rrd-java-client-v1.md` | Reconcile the generated Java projection against the shared SDK conformance boundary. |
| 16 | `docs/rrd-dotnet-client-v1.md` | Reconcile the generated .NET projection and close the SDK documentation set. |
| 17 | `docs/qdrant-capability-inventory.md` | Retain a source-pinned capability/reference inventory without importing Qdrant's product model. |
| 18 | `docs/surrealdb-capability-inventory.md` | Retain a source-pinned capability/reference inventory without importing SurrealDB's authority model. |
| 19 | `docs/rrflow-surrealdb-differential.md` | Preserve only reproducible claim-differential inputs and results after both source inventories are canonical. |
| 20 | `docs/anytype-ui-research.md` | Merge useful public-client/Connectome interaction requirements and remove UI product or lifecycle authority. |
| 21 | `docs/operations/ci.md` | Re-read the retained CI owner, verify its index and commands, and record the final KB-05 supporting-file disposition. |

After row 21, run the complete KB-05/A-06 acceptance corpus and change the
